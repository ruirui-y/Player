...
bool ObsNvencEncoder::GpuTextureToNv12(ID3D11Texture2D* texture)
{
    ...
    // ---- 第三步：BGRA → NV12（sws_scale 色彩转换） ----
    const uint8_t* src_data[] = { static_cast<uint8_t*>(mapped.pData) };
    ...
    if (!pixel_logged)
    {
        ...
        const uint8_t* p = static_cast<uint8_t*>(mapped.pData);
        ...
        qDebug("[ObsNvenc] staging 第100行首像素: %02X %02X %02X %02X",
               p[mapped.RowPitch * 100], p[mapped.RowPitch * 100 + 1],
               p[mapped.RowPitch * 100 + 2], p[mapped.RowPitch * 100 + 3]);
    }
